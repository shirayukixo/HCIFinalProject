	...
	function checkPuzzleSolved (fullList, foundWordsList, instructionsId) {
		...
		if ($(fullList).length == $(foundWordsList).length) {
			...
			if (selfSolved) {
				...
				//updates h2 text
				$(instructionsId).text("You got 'em all! :D. Please wait for five seconds...");
				setTimeout(() => {
			...
			//if user used the solve button 
				...
				//updates h2 text
				$(instructionsId).text("We solved it for you! :~). Please wait for five seconds...");
				setTimeout(() => {
					window.location.replace('solved.html');
				}, 5000);
